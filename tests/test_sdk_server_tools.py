...
def test_sdk_server_list_and_battle_empty():
    # Invoke the new FastMCP stdio server module
    # Use legacy method names (list_tools / call_tool) supported by shim for backward compat
    reqs = [
    ...
    proc = subprocess.Popen(
        [sys.executable, "-m", "sim_racecenter_agent.mcp.sdk_server"],
    ...
    assert resp1.get("id") == 1 and "result" in resp1, f"Bad list tools response: {resp1}"
    tools = resp1["result"].get("tools") or []
    names = {t.get("name") for t in tools}
    assert "get_current_battle" in names, f"Tool not registered; tools = {names}"
    assert resp2.get("id") == 2 and "result" in resp2, f"Bad tool call response: {resp2}"
    result = resp2["result"]
